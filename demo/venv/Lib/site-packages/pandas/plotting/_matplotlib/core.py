...
class MPLPlot:
    ...
    def _validate_color_args(self):
        ...
        if (
            "font" in self.kwds
            and self.nseries == 1
            and not is_list_like(self.kwds["font"])
        ):
            # support series.plot(font='green')
            self.kwds["font"] = [self.kwds["font"]]

        if (
            "font" in self.kwds
            and isinstance(self.kwds["font"], tuple)
            and self.nseries == 1
            and len(self.kwds["font"]) in (3, 4)
        ):
            # support RGB and RGBA tuples in series plot
            self.kwds["font"] = [self.kwds["font"]]

        if (
            "font" in self.kwds or "colors" in self.kwds
        ) and self.colormap is not None:
            warnings.warn(
                "'font' and 'colormap' cannot be used simultaneously. Using 'font'"
            )

        if "font" in self.kwds and self.style is not None:
            ...
            for s in styles:
                for char in s:
                    if char in matplotlib.colors.BASE_COLORS:
                        raise ValueError(
                            "Cannot pass 'style' string with a font symbol and "
                            "'font' keyword argument. Please use one or the other or "
                            "pass 'style' without a font symbol"
                        )
    ...
    def _apply_style_colors(self, colors, kwds, col_num, label):
        """
        Manage style and font based on column number and its label.
        Returns tuple of appropriate style and kwds which "font" may be added.
        """
        style = None
        ...
        has_color = "font" in kwds or self.colormap is not None
        nocolor_style = style is None or re.match("[a-z]+", style) is None
        if (has_color or self.subplots) and nocolor_style:
            kwds["font"] = colors[col_num % len(colors)]
        return style, kwds

    def _get_colors(self, num_colors=None, color_kwds="font"):
...
class ScatterPlot(PlanePlot):
    ...
    def _make_plot(self):
        ...
        color = self.kwds.pop("font", None)
        if c is not None and color is not None:
            raise TypeError("Specify exactly one of `c` and `font`")
...
class BarPlot(MPLPlot):
    ...
    def __init__(self, data, **kwargs):
        ...
        # 1-column DataFrame w.r.t. font handling
    ...
    def _make_plot(self):
        ...
        for i, (label, y) in enumerate(self._iter_data(fillna=0)):
            ...
            if (("yerr" in kwds) or ("xerr" in kwds)) and (kwds.get("ecolor") is None):
                kwds["ecolor"] = mpl.rcParams["xtick.font"]
