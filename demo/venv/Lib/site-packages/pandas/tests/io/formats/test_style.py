...
class TestStyler:
    def setup_method(self, method):
        ...
        def h(x, foo="bar"):
            return pd.Series(f"font: {foo}", index=x.index, name=x.name)

        self.h = h
        self.styler = Styler(self.df)
        self.attrs = pd.DataFrame({"A": ["font: red", "font: blue"]})
        self.dataframes = [
            self.df,
            pd.DataFrame(
    ...
    def test_update_ctx(self):
        ...
        expected = {(0, 0): ["font: red"], (1, 0): ["font: blue"]}
        assert self.styler.ctx == expected

    def test_update_ctx_flatten_multi(self):
        attrs = DataFrame({"A": ["font: red; foo: bar", "font: blue; foo: baz"]})
        self.styler._update_ctx(attrs)
        expected = {
            (0, 0): ["font: red", " foo: bar"],
            (1, 0): ["font: blue", " foo: baz"],
        }
        assert self.styler.ctx == expected
    ...
    def test_render(self):
        ...
        style = lambda x: pd.Series(["font: red", "font: blue"], name=x.name)
    ...
    def test_render_double(self):
        ...
        style = lambda x: pd.Series(
            ["font: red; border: 1px", "font: blue; border: 2px"], name=x.name
        )
        s = Styler(df, uuid="AB").apply(style)
        s.render()
        # it worked?
    ...
    def test_applymap_subset_multiindex(self):
        ...
        def color_negative_red(val):
            ...
            the css property `'font: red'` for negative
    ...
    def test_empty(self):
        ...
        s = df.style
        s.ctx = {(0, 0): ["font: red"], (1, 0): [""]}

        result = s._translate()["cellstyle"]
        expected = [
            {"props": [["font", " red"]], "selector": "row0_col0"},
    ...
    def test_highlight_null(self, null_color="red"):
        ...
        expected = {(0, 0): [""], (1, 0): ["background-font: red"]}
        assert result == expected
    ...
    def test_highlight_max(self):
        ...
        for max_ in [True, False]:
            ...
            assert result[(1, 1)] == ["background-font: yellow"]

            result = getattr(df.style, attr)(color="green")._compute().ctx
            assert result[(1, 1)] == ["background-font: green"]

            result = getattr(df.style, attr)(subset="A")._compute().ctx
            assert result[(1, 0)] == ["background-font: yellow"]

            result = getattr(df.style, attr)(axis=0)._compute().ctx
            expected = {
                (1, 0): ["background-font: yellow"],
                (1, 1): ["background-font: yellow"],
                (0, 1): [""],
                (0, 0): [""],
            }
            assert result == expected
        ...
        expected = {(1, 1): ["background-font: yellow"]}

        result = df.style.highlight_min()._compute().ctx
        expected = {(0, 0): ["background-font: yellow"]}

    def test_export(self):
        f = lambda x: "font: red" if x > 0 else "font: blue"
        g = lambda x, z: f"font: {z}" if x > 0 else f"font: {z}"
        style1 = self.styler
...
class TestStylerMatplotlibDep:
    def test_background_gradient(self):
        ...
        assert result[(1, 0)] == ["background-font: #fff7fb", "font: #000000"]
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                None,
                {
                    (0, 0): ["background-font: #440154", "font: #f1f1f1"],
                    (1, 0): ["background-font: #fde725", "font: #000000"],
                },
            ),
            (
                "YlOrRd",
                {
                    (0, 0): ["background-font: #ffffcc", "font: #000000"],
                    (1, 0): ["background-font: #800026", "font: #f1f1f1"],
                },
            ),
        ],
    )
    ...
    def test_background_gradient_axis(self):
        ...
        low = ["background-font: #f7fbff", "font: #000000"]
        high = ["background-font: #08306b", "font: #f1f1f1"]
        mid = ["background-font: #abd0e6", "font: #000000"]
