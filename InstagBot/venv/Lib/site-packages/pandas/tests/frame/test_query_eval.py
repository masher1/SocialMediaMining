...
class TestDataFrameQueryWithMultiIndex:
    def test_query_with_named_multiindex(self, parser, engine):
        ...
        index = MultiIndex.from_arrays([a, b], names=["font", "food"])
        df = DataFrame(np.random.randn(10, 2), index=index)
        ind = Series(
            df.index.get_level_values("font").values, index=index, name="font"
        )

        # equality
        res1 = df.query('font == "red"', parser=parser, engine=engine)
        res2 = df.query('"red" == font', parser=parser, engine=engine)
        exp = df[ind == "red"]
        ...
        # in/not in ops
        res1 = df.query('["red"] in font', parser=parser, engine=engine)
        res2 = df.query('"red" in font', parser=parser, engine=engine)
        exp = df[ind.isin(["red"])]
        tm.assert_frame_equal(res1, exp)
        tm.assert_frame_equal(res2, exp)

        res1 = df.query('["red"] not in font', parser=parser, engine=engine)
        res2 = df.query('"red" not in font', parser=parser, engine=engine)
        exp = df[~ind.isin(["red"])]
        tm.assert_frame_equal(res1, exp)
        tm.assert_frame_equal(res2, exp)
