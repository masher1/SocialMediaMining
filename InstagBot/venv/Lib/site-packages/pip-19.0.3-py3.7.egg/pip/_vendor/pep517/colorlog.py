...
class LogFormatter(logging.Formatter):
    ...
    def __init__(self, color=True, datefmt=None):
        r"""
        :arg bool color: Enables font support.
        :arg string fmt: Log message format.
        It will be applied to the attributes dict of log records. The
        text between ``%(font)s`` and ``%(end_color)s`` will be colored
        depending on the level if font support is on.
        :arg dict colors: font mappings from logging level to terminal font
        code
